...
void init_highscore(int *Highscore, int *suspension){
	...
	if(save_highscore != NULL){// Si l'ouverture réussie.
		int h;
		j = 0;
		char ligne[50];
		while(j<5 && fgets(ligne,50,save_highscore) != NULL){ 
		// j<5 également car la dernière ligne contient du vide et pas NULL, à cause du '\n'.
			
			for(h=0;h<11;h++){ // On vide temp.
				temp[h] = '\0';
			}
			i = 0;
			while(ligne[i] != ';'){// On lit le ième highscore.
				temp[i] = ligne[i];
				i += 1;
			}
			Highscore[j] = atoi(temp); // On ecrit le ième score convertit dans le tableau.
...
void Save_highscore(int *Highscore){
	FILE *save_highscore = NULL;
	save_highscore = fopen("highscore.txt","w");// Le fichier est créé si inéxistant.
	...
	while(i<5){// Tant qu'il reste des highscore.
		n = 0;
		i_score = Highscore[i]; // ième score.
		
		if(i_score == 0){ // Si le ième highscore est nul.
			n = 1;
		}
		
		for(j=0;j<11;j++){ // On vide le tableau rempli précédemment.
			nb_coma[j] = '\0';
		}
		
		// le nombre de ';' qu'il faudra en fonction du nombre de caractères de du ième score.
		while(i_score%10 != 0 || i_score/10 != 0){
			n += 1;
			i_score = i_score / 10;
		}
		for(j=0;j < 11-n;j++){// On rempli le tableau de ';' nécéssaire.
			nb_coma[j] = ';';
		}
...
void change_highscore(int score,int *Highscore){
	...
	for(i=0;i<5;i++){
		if(score > Highscore[i]){// Si nouveau record.
			...
			i +=1;
			for(i;i<5;i++){ // On déplace les scores en fonction du nouveau record.
				temp2 = Highscore[i];
				Highscore[i] = temp1;
				temp1 = temp2;
			}
...
void identification(player_t *player){
	FILE* save = NULL;
	int load; // Vaut 1 si le joueur a été chargé. 0 sinon.
	...
	else{// Si il n'existe pas on créer le fichier (vide).
		save = fopen("save.txt","w");
		fclose(save);
 	}
 	
 	if(load == 0){ // Si le joueur n'a pas été chargé.
		char temp[10];
		int numb;
		while(player->money < 0){
			...
			while(numb <= 0){ // Si réponse inconvenable.
...
int read_save(FILE* save, player_t *player){
	...
	int i,not_idem = 1; // not_idem vaut 1 si le nom du joueur n'est pas trouvé, 0 sinon.
	
	while(not_idem && fgets(ligne,50,save) != NULL){
		// On lit chaque ligne du fichier si le joueur n'est pas trouvé.
		not_idem = 0;
		for(i=0;name[i]!='\0';i++){
			if(ligne[i] != name[i]){
		...
		if(not_idem == 0){ // On vérifie que le nom de la ligne est bien fini.
			if(ligne[i] != ','){
				not_idem = 1;
			}
		}
	...
	if(not_idem){// Le joueur n'as pas été trouvé.
		return 0;
	}
	else{
		char rep[2];
		...
		if(*rep == 'o'){
			...
			int j=0;
			i++;
			for(i;ligne[i] != ';';i++){// On lit l'argent sauvegardé sur la ligne actuelle.
				temp[j] = ligne[i];
				j++;
			}
			player->money = atoi(temp);// On le convertit en entier. 
			return 1;// Le joueur a été trouvé.
		}
		else{
...
void write_save(player_t *player){
	...
	char ligne[50];
	char *name = player->name;
	int money = player->money; // money va être modifié.
	int i,not_idem = 1;
	...
	while(not_idem && fgets(ligne,45,save) != NULL){
		// On lit chaque ligne du fichier si le joueur n'est pas trouvé.
		currnt_curst = ftell(save);// Position du curseur à la fin de la ligne actuelle.
		taille = currnt_curst - old_curs;// taille de la ligne actuelle.
		old_curs = currnt_curst;// Position du curseur à la fin de la ligne précédente.
	...
	if(not_idem){//Le joueur n'existait pas, on l'ecrit dans save.txt.
		// avec 0 chiffre il y a 10 ";" pour amortir l'écrasement des futurs write (le somme peut faire jusqu'à 9 caractères).
		int n = 0;
		...
		char nb_coma[11];// le nombre de ';' qu'il faudra en fonction du nombre de caractères de money.
		...
		for(i=0;i < 10-n;i++){ // On place le nombre de ';' nécéssaire.
			nb_coma[i] = ';';
		}
		...
		fseek(save,-taille,SEEK_CUR); // On revient au début de la ligne actuelle.
		fprintf(save,"%s,%d;",name,money);// On ecrit la nouvelle somme d'argent du joueur.
		//";" à la fin pour couper si on passe de 1000 à 50 par exemple. (sinon on aurait 5000$)
	}
	fclose(save);
}
...
void player_add(player_t **addr_first){
	...
	add_player->suivant = NULL; // Insertion du nouveau joueur à la fin de la liste chainée.
	curr_player->suivant = add_player;
}

void player_leave(player_t **player, player_t **addr_first, player_t **ptr_lanceur, int *Continuer){
	player_t *to_delete = *player; // Le joueur à effacer.
	char rep[2];
	...
	if(*rep == 'o'){
		...
		printf("Au revoir %s.\n",to_delete->name);
		delete_player(to_delete, addr_first); // On le retire de la liste chainée.
		
		//Si c'était le lanceur actuel.
		if(*player == *ptr_lanceur){
			if((*player)->suivant != NULL){ // Si ce n'était pas le dernier.
				*ptr_lanceur =(*ptr_lanceur)->suivant;
				printf("\n*** Le lanceur est a present %s ***\n",(*ptr_lanceur)->name);
			}
			else{// Si c'était le dernier.
				*ptr_lanceur = *addr_first; // On retourne au début de la liste chainée.
				if((*ptr_lanceur)->suivant != NULL){ // Si le joueur n'était pas seul.
					printf("\n*** Le lanceur est a present %s ***\n",(*ptr_lanceur)->name);
				}
			}
		...
		if(*player == *addr_first){ // Si le joueur était le premier de la liste.
			if((*player)->suivant != NULL){ // Si il n'était pas seul.
				*player = (*player)->suivant;
				*addr_first = (*addr_first)->suivant; // Le premier est maintenant le deuxième de la liste.
			}else{
				*Continuer = 0; // Il était seul. On arrete la boucle principale.
			}
		}else{// Sinon on passe au suivant.
			if((*player)->suivant != NULL){
				*player = (*player)->suivant;
			}else{// On retourne au début de la liste chainée.
				*player = *addr_first;
				
			}
		}
		
		free(to_delete); // On free le joueur qui a quitté la partie.
	}	
}
...
void delete_player(player_t *player, player_t **addr_first){
	...
	if(player != *addr_first){
		...
		// On parcours la liste jusqu'au joueur à supprimer en mémorisant le précédent.
			old_current = current_player; 
			current_player = current_player->suivant;
		}
		//On supprime le joueur de la liste chainée.
...
void player_change_bet(player_t *player, int suspension, int *changed){
	...
	if(suspension == 0){ // Si le point viens d'être fait.
		...
		if(player->player_line == PASS){ // Il peut augmenter sa mise.
			...
			if(*rep == 'o' || *rep =='O'){
				...
				while(newmise < 0 || newmise > player->money){ // Si la mise à ajouter est valide.
					printf("Veuillez saisir une valeur correcte et inferieure a votre argent actuel.\n>> ");
					fscanf(stdin,"%9s",temp);
					newmise = atoi(temp);
				}
			...
			if(*rep == 'o' || *rep =='O'){
				...
				while(newmise < 0 || newmise > player->bet){// Si la mise à ajouter est valide.
	...
	else if(suspension == 1){ // Si il s'agit de la première suspension.
		if(player->player_line == PASS){
			printf("%s avez parie %d$ sur PassLine et vous avez %d$\n",player->name,player->bet,player->money);
		}else{
			printf("%s avez parie %d$ sur Don't PassLine et vous avez %d$\n",player->name,player->bet,player->money);
	...
	else{// Ce n'est pas la première suspension.
		if(player->player_line == PASS){
			printf("%s avez parie %d$ sur PassLine et vous avez %d$\n",player->name,player->bet,player->money);
		}else{
			printf("%s avez parie %d$ sur Don't PassLine et vous avez %d$\n",player->name,player->bet,player->money);
		...
		fscanf(stdin,"%2s",rep);
		if(*rep == 'o' || *rep =='O'){// Il peut diviser sa mise par deux (et récuperer l'argent retiré).
...
void first_phase(player_t *player, int point_tmp, int *Highscore){
	if(player->player_line == PASS){ // Si le joueur a misé sur Passline.
	...
	else if(player->player_line == DONTPASS){// Si le joueur a misé sur Don't Passline.
